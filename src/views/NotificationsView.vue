...
<template>
   ...
   <div>
     <ul>
        <li v-for="notification in notifications" :key="notification.id" class="flex flex-col border-b space-y-5 p-6" :class="notification.status === 'pending' ? 'bg-brand-cornsilk' : 'bg-white'">
            ...
            <div class="flex flex-col space-y-2">
                ...
                <div>Utilizatorul <span>{{ notification.fromUserEmail }}</span> te-a invitat sa devii colaborator la acest proiect.</div>
                ...
                <div v-if="notification.status === 'pending'" class="flex space-x-3">
                     <button @click="acceptInvitation(notification.id, notification.toUserId, notification.projectId)"
